...
func NewController(kubeclientset kubernetes.Interface,
	...
	ingressInfromer netv1informer.IngressInformer,
	...
	controller := &Controller{
		...
		ingressSynced: 	   ingressInfromer.Informer().HasSynced,
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.serviceSynced, c.ingressSynced, c.xcrdSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	ingressName := xcrd.Spec.Name+"-ing"

	ingress, err := c.ingressLister.Ingresses(namespace).Get(ingressName)

	if errors.IsNotFound(err) {
		ingress, err = c.kubeclientset.NetworkingV1().Ingresses(xcrd.Namespace).Create(context.TODO(),newIngress(xcrd),metav1.CreateOptions{})
	}
